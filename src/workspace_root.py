...
# Mythic: Locate the workshop floor from a module inside src/.
# Engineering: Walk parents of module_file until repo markers; else return src's parent.
# Invariant: returned Path is absolute (resolved from the start file).
# Boundary: does not read env overrides or reject filesystem root.
...
# Mythic: Stake the operator workshop boundary so local tools never walk the machine.
# Engineering: Prefer env override, then explicit root, then inferred repo root; reject `/` and non-dirs.
# Invariant: returned Path is never filesystem root (`/`).
# Boundary: read-only resolution; does not create directories or mutate env.
